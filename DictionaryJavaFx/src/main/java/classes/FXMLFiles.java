package classes;

public class FXMLFiles {
    public static String UPDATE_WORD_SCENE = "UpdateWordScene.fxml";
    public static String MAIN_UI_SCENE = "NewMainUIScene.fxml";
    public static String EXPERIMENT_GAME_SCENE = "ExperimentGameScene.fxml";
    public static String SHOOTING_GAME_SCENE = "ShootingGameScene.fxml";
    public static String INSERT_WORDS_SCENE =  "InsertWordsScene.fxml";
    public static String TRANSLATE_TEXT_SCENE = "translateTextScene.fxml";
    public static String LOADING_SCENE = "LoadingScene.fxml";
    public static String BOOK_MARK_SCENE = "BookMarkScene.fxml";
}
